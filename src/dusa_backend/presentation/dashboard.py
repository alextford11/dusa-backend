...
from src.dusa_backend.infrastructure.schemas.dashboards import DashboardStatsResponse
...
async def get_dashboard_stats(db_session: Session = Depends(get_db)) -> DashboardStatsResponse:
    ...
    todays_categories = category_repo.get_todays_stats(limit=5)
    yesterdays_categories = category_repo.get_yesterdays_stats(limit=5)
    all_time_categories = category_repo.get_all_time_stats(limit=5)
