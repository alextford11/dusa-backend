...
from src.dusa_backend.infrastructure.errors.base import BaseError
from src.dusa_backend.infrastructure.models import ErrorResponse, ErrorResponseMulti


def custom_base_errors_handler(_: Request, error: BaseError) -> JSONResponse:
    """This function is called if the BaseError was raised."""

    response = ErrorResponseMulti(results=[ErrorResponse(message=error.message.capitalize())])
